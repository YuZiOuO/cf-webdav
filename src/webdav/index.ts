...
app.use("*", async (c, next) => {
  try {
    ...
    const locks = new Locks(filesystem);
    const properties = new Properties(filesystem, [
      ...quotaProtectedPropertyNames,
      ...syncProtectedPropertyNames,
    ]);
    const resource = (target: Path) => new Resource(target, filesystem);
    const sync = new Sync(resource, filesystem);
    c.set("path", path);
    c.set("dav", {
      resource,
      locks,
